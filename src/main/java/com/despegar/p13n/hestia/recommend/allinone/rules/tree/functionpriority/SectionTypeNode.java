...
public class SectionTypeNode
    extends DecisionNode<SectionFunctionContent> {

    private Map<SectionType, Node<SectionFunctionContent>> childs = Maps.newLinkedHashMap();
    ...
    public Value<SectionFunctionContent> add(RuleDef ruleDef, SectionFunctionContent function) {
        ...
        if (!this.childs.containsKey(rule.getSection())) {
            this.childs.put(rule.getSection(), this.getNewChild());
        }

        return this.childs.get(rule.getSection()).add(rule, function);
    }
    ...
    public Node<SectionFunctionContent> getNext(Query query) {
        ...
        return this.childs.get(q.getSection());
    }
    ...
    public boolean containsRule(RuleDef ruleDef) {
        ...
        Node<SectionFunctionContent> node = this.childs.get(rule.getSection());
